...
class FloorPlanAgent(BaseNode[MainState]):
    async def run(self, ctx: GraphRunContext[MainState]) -> DesignLoopEntry:
        ...
        if is_debug:
            ...
            # Add generated rooms to scene
            console.print("[bold cyan]Adding generated rooms to scene...[/]")
            
            try:
                for room in generated_rooms:
                    # Note: room.boundary is already generated by the LLM with the requested shape
                    console.print(f"[bold green]✓ Added room {room.id} with {len(room.boundary) if room.boundary else 0} boundary points[/]")
                        
            except Exception as room_error:
                console.print(f"[bold red]✗ Error processing room: {room_error}[/]")
                raise room_error
