...
def test_save_lint_visualization_creates_image(tmp_path):
    intruding = _make_object("intrude", position=(3.9, 2.0, 0.0), scale=(1.0, 1.0, 1.0))
    room = _square_room([intruding])

    report = lint_room(room, size_provider=_world_bounds_from_scale)
    output = tmp_path / "lint.png"

    save_lint_visualization(
        room,
        report,
        output,
        size_provider=_world_bounds_from_scale,
    )
